...
def core(fixup_prefix = "") -> None:
	args = sys.argv[1:]
	flags = []
	rebase_merges = git_run(["config", "--get", "fixupCommands.rebaseMerges"], subprocess.PIPE).strip().lower()

	if rebase_merges in ("", "true"):
		flags.append("--rebase-merges")
	elif rebase_merges == "false":
		flags.append("--no-rebase-merges")
	else:
		flags.append("--rebase-merges=" + rebase_merges)
	...
	git_run(["rebase", "--autosquash", "--autostash", *flags, parent_ref], env=rebase_env, capture_stdout=True)
